# 🎨 VS Code Extension Guide# 🎨 VS Code Extension Guide# VS Code Extension Guide
...
### Method 1: Install from VSIX (Recommended)## 📦 Installation## Table of Contents



1. **Locate Extension File**- [Features](#features)

   ```bash

   # In PowerScript repository### Method 1: Install from VSIX (Recommended)- [Installation](#installation)

   cd vscode-extension

   # File: powerscript-1.0.0.vsix (550.28 KB)- [Getting Started](#getting-started)
...
2. **Install in VS Code**
   ...
   - Open VS Code   ```bash- [Commands](#commands)

   - Press `Cmd+Shift+P` (Mac) or `Ctrl+Shift+P` (Windows/Linux)

   - Type: `Extensions: Install from VSIX`   # In PowerScript repository- [Keyboard Shortcuts](#keyboard-shortcuts)
...
3. **Verify Installation**   # File: powerscript-1.0.0.vsix (550.28 KB)- [Troubleshooting](#troubleshooting)
...
cd vscode-extension   - Open VS Code### ✨ Syntax Highlighting



# Install Node.js dependencies   - Press `Cmd+Shift+P` (Mac) or `Ctrl+Shift+P` (Windows/Linux)- Full syntax highlighting for `.ps` files
...
npm run compile   - Navigate to `powerscript-1.0.0.vsix`- Support for modern PowerScript features



# Package extension   - Click "Install"- Semantic highlighting for better readability
...
# Install the generated .vsix3. **Verify Installation**### 🧠 IntelliSense
...
## ✨ Features   - Check Extensions panel for "PowerScript"- Variable name completion



### 1. Syntax Highlighting ✅- Import statement assistance



Beautiful, semantic highlighting for PowerScript code:### Method 2: Build from Source- Parameter hints
...
- Classes: `class`, `interface`, `abstract`, `extends`, `implements`cd vscode-extension- Real-time syntax error detection

- Functions: `function`, `return`, `constructor`

- Modules: `import`, `export`, `from`, `as`- Type checking warnings



**Operators & Symbols:**# Install Node.js dependencies- Missing semicolon suggestions

- Arithmetic: `+`, `-`, `*`, `/`, `%`

- Comparison: `==`, `!=`, `<`, `>`, `<=`, `>=`npm install- Unknown type flags

- Logical: `&&`, `||`, `!`

- Assignment: `=`, `+=`, `-=`, `*=`, `/=`- Inline error messages



**Literals:**# Compile TypeScript

- Strings: `"double quotes"`, `'single quotes'`

- Numbers: `42`, `3.14`, `0xFF`npm run compile### 📝 Code Snippets
...
### 2. Code Snippets ✅# Package extension- `function` - Function declaration
...
#### Basic Snippetsvsce package- `interface` - Interface declaration



**`func`** - Function Declaration- `arrow` - Arrow function
...
class ClassName {- `import` - Import statement
...
interface InterfaceName {
    ...
    property: type;- Classes: `class`, `interface`, `abstract`, `extends`, `implements`
...
```powerscript- Numbers: `42`, `3.14`, `0xFF`

for (let i = 0; i < length; i++) {

    // loop body- Booleans: `true`, `false`## Installation
...
**`while`** - While Loop### Method 1: Install from GitHub Release
...
switch (expression) {
    ...
    case value2:
        ...
        // case 2**`func`** - Function Declaration2. **Download the Extension**

        break;

    default:```powerscript   - Navigate to the `vscode-extension/` folder
...
```powerscript```3. **Install in VS Code**
...
async function functionName(): Promise<type> {
    ...
    // async body# Clone the repository
...
**`enum`** - Enum Declaration```powerscriptcd Python-PowerScript/vscode-extension

```powerscript

enum EnumName {class ClassName {

    Value1,

    Value2,    constructor(param: type) {# Install dependencies
...
let x: number = 5; // inline comment**`interface`** - Interface Declarationcode --install-extension powerscript-1.0.0.vsix
 ...
 * This is a    property: type;### Method 3: Install via VS Code UI
...
- Block comment: `Cmd+Shift+A` (Mac) or `Ctrl+Shift+A` (Windows/Linux)#### Control Flow Snippets4. Select the downloaded `.vsix` file
...
- Select text and type `{` → Wraps in `{}`}2. Go to Extensions (`Ctrl+Shift+X`)
...
2. **Press Tab** to expand} else {

3. **Tab through placeholders** to fill in values

4. **Press Enter** when done    // false block1. Create a new file with `.ps` extension



### Example: Create a Class}2. VS Code automatically activates PowerScript mode
...
4. Tab to type: string**`for`** - For Loop### First PowerScript File

5. Tab to method: greet

6. Tab to return type: string```powerscript
